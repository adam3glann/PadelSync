...
function result(task) {
  return task
    .then(function (data) {
      return { ok: true, data: data, message: data.message };
    })
...
window.db = {
  ...
  getMyBookings: function (page, limit) {
    return request(
      "/bookings/mine?page=" + (page || 1) + "&limit=" + (limit || 20),
    );
  },
  getAllBookings: function (date, page, limit) {
    return request(
      "/bookings?status=confirmed&date=" +
  ...
  getCancellations: function () {
    return request("/bookings?status=cancelled&page=1&limit=50").then(
      function (data) {
        return data.data;
      },
    );
