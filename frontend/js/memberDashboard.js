...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (user) {
    var welcomeText = (window.i18n ? window.i18n.t('dash.welcomeBack') : 'Welcome back') + ', <span class="gradient-text">' + escapeHtml(user.fullName || user.name || '') + '</span>!';
...
function renderUpcoming(bookings) {
  ...
  for (var i = 0; i < bookings.length; i++) {
    ...
    html += '<div><div class="ticket-card">' +
      ...
      '<div class="ticket-title">' + escapeHtml(b.court ? b.court.name : 'Court unavailable') + '</div>' +
