  ...
  try {
    ...
    if (slots.length === 0 && bookingsPage === 1) {
      tbody.innerHTML =
        ...
        (window.i18n
          ? window.i18n.t("bookings.noBookings")
    ...
    for (var i = 0; i < slots.length; i++) {
      var slot = slots[i];
      var courtName = slot.court
        ? slot.court.name
        : window.i18n
          ? window.i18n.t("bookings.unknown")
          : "Court unavailable";
      var equipmentDisplay;
      if (slot.equipment && slot.equipment !== "None") {
        equipmentDisplay =
          '<span class="badge badge-equipment">' +
          escapeHtml(slot.equipment) +
          "</span>";
      } else {
        equipmentDisplay =
          '<span style="color: var(--text-muted);">' +
          (window.i18n ? window.i18n.t("bookings.none") : "None") +
          "</span>";
      }

      var playerName = slot.user
        ? escapeHtml(slot.user.fullName)
        : window.i18n
          ? window.i18n.t("bookings.unknown")
          : "Unknown";
      ...
      rows +=
        "<tr>" +
        '<td style="color: var(--padel-blue); font-weight: 500;">' +
        ...
        equipmentDisplay +
        "</td>" +
        '<td><button class="btn btn-danger" style="padding: 0.3rem 0.6rem; font-size: 0.8rem;" onclick="adminCancel(\'' +
        slot._id +
        "')\">" +
        cancelLabel +
        "</button></td>" +
    ...
    if (pagEl) {
      if (pagination && pagination.totalPages > 1) {
        renderPagination("bookings-pagination", pagination, function (p) {
          fetchBookings(date, p);
        });
      } else {
        pagEl.innerHTML = "";
      }
    ...
    tbody.innerHTML =
      '<tr><td colspan="7" class="text-center" style="color: var(--danger);">Failed to load bookings.</td></tr>';
  }
}
...
function adminCancel(id) {
  var title = window.i18n
    ? window.i18n.t("modal.cancelMemberBooking")
    : "Cancel Member Booking";
  var msg = window.i18n
    ? window.i18n.t("modal.cancelMemberMsg")
...
async function fetchCancelledSchedules() {
  ...
  try {
    ...
    total.textContent =
      records.length + (records.length === 1 ? " cancelled" : " cancelled");

    if (records.length === 0) {
      body.innerHTML =
        '<tr><td colspan="7" class="text-center">No cancelled schedules yet.</td></tr>';
      return;
    }
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      var courtName = record.court
        ? escapeHtml(record.court.name)
        : window.i18n
          ? window.i18n.t("bookings.unknown")
          : "Court unavailable";
      var memberName = record.user
        ? escapeHtml(record.user.fullName)
        : "Unknown";
      var memberEmail = record.user ? escapeHtml(record.user.email) : "-";
      rows +=
        "<tr>" +
        '<td style="color:var(--padel-blue);font-weight:bold;">' +
        courtName +
        "</td>" +
        "<td>" +
        record.date +
        ...
        "</td>" +
        '<td class="refund-value">EGP ' +
        record.refundAmount +
        "</td>" +
        '<td class="retained-value">EGP ' +
        ((record.depositAmount || 0) - (record.refundAmount || 0)) +
        "</td>" +
        "<td>" +
        (record.cancelledAt
    ...
    body.innerHTML =
      '<tr><td colspan="7" class="text-center" style="color: var(--danger);">Failed to load cancelled schedules.</td></tr>';
  }
}
