...
// Prevent double-booking: only one *confirmed* booking may exist per
// court/date/block. Scoped to status "confirmed" (a partial index) so that
// cancelling a booking actually frees the slot back up — a plain unique
// index here would keep blocking that slot forever, since cancelling just
// flips status on the existing document instead of deleting it.
bookingSchema.index(
