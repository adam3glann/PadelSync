...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (user) {
    ...
    var dashLink =
      user.role === "admin" ? "admin/dashboard.html" : "member/dashboard.html";
    navLinks.innerHTML =
      auth.renderThemeButton() +
      '<a href="' +
      dashLink +
      '" data-i18n="nav.dashboard">Dashboard</a>' +
    ...
    ctaContainer.innerHTML =
      '<a href="' +
      dashLink +
      '" class="btn btn-primary" style="font-size: 1.1rem; padding: 1rem 2.5rem;" data-i18n="hero.goDashboard">Go to Dashboard</a>';
  } else {
  ...
  if (auth.toggleTheme) {
    ...
    if (themeButton) {
      themeButton.querySelector(".theme-icon").textContent = isLight
        ? "☾"
        : "☀";
      themeButton.querySelector(".theme-label").textContent = isLight
        ? "Dark"
        : "Light";
    }
  }
