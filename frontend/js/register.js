...
if (_user && _user.role) {
  window.location.href =
    _user.role === "admin" ? "admin/dashboard.html" : "member/dashboard.html";
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  registerForm.addEventListener("submit", async function (e) {
    ...
    btn.textContent = window.i18n
      ? window.i18n.t("auth.creatingAccount")
      : "Creating account...";

    // Try to register
    var result = await db.register(
    ...
    auth.showToast(
      window.i18n ? window.i18n.t("toast.accountCreated") : "Account created!",
    );
    setTimeout(function () {
