...
export const getSlots = async (req, res, next) => {
  try {
    ...
    const [courts, bookings] = await Promise.all([
      Court.find({}),
      Booking.find({ date, status: "confirmed" }).populate(
        "user",
        "fullName email",
      ),
    ]);
    const bookingMap = new Map(
      bookings.map((booking) => [
        `${booking.court}|${booking.timeBlock}`,
        booking,
      ]),
    );
...
export const createBooking = async (req, res, next) => {
  try {
    ...
    if (
      !courtId ||
      !isValidDate(date, { maxDays: 30 }) ||
      !TIME_BLOCKS.includes(timeBlock)
    ) {
      return res.status(400).json({
        message: "Court, a valid upcoming date, and a time block are required.",
      });
    }
    ...
    try {
      ...
      res.status(201).json({
        message: "Demo payment successful. Your booking is confirmed.",
        booking,
      });
    } catch (err) {
      if (err.code === 11000) {
        return res
          .status(409)
          .json({ message: "This time slot was just booked by someone else." });
      }
      throw err;
    }
  } catch (error) {
...
export const getBookings = async (req, res, next) => {
  try {
    ...
    if (
      req.query.status &&
      ["confirmed", "cancelled"].includes(req.query.status)
    )
...
export const cancelBooking = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "You can only cancel your own booking." });
    ...
    if (startMinutes !== null) {
      ...
      else if (hoursUntilPlay > 0)
        refundAmount = Math.round(booking.depositAmount * 0.25 * 100) / 100;
    }
    ...
    res.json({
      message: refundAmount
        ...
        : "Booking cancelled. The deposit is not refundable within 2 hours of the match.",
      refundAmount,
    });
  } catch (error) {
    next(error);
...
export const getStats = async (req, res, next) => {
  try {
    const [courts, members, bookings, cancellations] = await Promise.all([
      Court.find(),
    ...
    const cashDue = bookings.reduce(
      (sum, booking) => sum + (booking.cashAmount || 0),
      0,
    );
    const refunds = cancellations.reduce(
